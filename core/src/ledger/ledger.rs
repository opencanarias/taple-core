...
impl<C: DatabaseCollection> Ledger<C> {
                ...
                match ledger_state {
                    Some(ledger_state) => {
                        match ledger_state.head {
                            Some(head) => {
                                // Si hay un LCE es un error, comprobar si es el mismo evento y si no fallo
                            }
                            None => {
                                // Comprobar que es el siguiente evento o si es LCE
                            }
                        }
                    }
                    None => {
    ...
    pub async fn get_next_gov(
        ...
        let subject = self.database.get_subject(&subject_id)?;
        let event = self.database.get_event(&subject_id, sn)?;
        let signatures = match self.database.get_signatures(&subject_id, sn) {
            Ok(s) => s,
            Err(error) => match error {
