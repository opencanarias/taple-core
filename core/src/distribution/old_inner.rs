...
use crate::commons::self_signature_manager::{SelfSignatureManager, SelfSignatureInterface};
use crate::identifier::{Derivable, DigestIdentifier, KeyIdentifier};
use crate::message::{MessageConfig, MessageTaskCommand};
use crate::Notification;
use crate::{
    database::DB, governance::GovernanceInterface, DatabaseManager, Event,
};
    ...
    InnerDistributionManager<D, G, N>
{
    ...
    async fn add_new_event(
        ...
        if let Err(error) = self.db.apply_event_sourcing(&event.content) {
            return Err(DistributionManagerError::DatabaseError(error.to_string()));
        }
        Ok(())
    }
    ...
    async fn add_new_event_with_signature(
        ...
        let Ok(own_signature) = self.signature_manager.sign(&event.content) else {
    ...
    async fn add_new_subject(
        ...
        if let Err(error) = self
            .db
            .set_event(&event.content.subject_id, event.clone()) {
            return Err(DistributionManagerError::DatabaseError(error.to_string()));
        }
        if let Err(error) = self.db.set_signatures(
            &event.content.subject_id,
        ...
        let subject_schema = match self
            .governance
            .get_schema(
                &event.content.metadata.governance_id,
                &event.content.metadata.schema_id,
            )
            .await
        {
            Ok(result) => result,
        ...
        if let Err(error) = self
            .db
            .set_subject(&event.content.subject_id.clone(), subject)
        {
            return Err(DistributionManagerError::DatabaseError(error.to_string()));
        }
        Ok(Ok(()))
    ...
    pub async fn signature_received(
        ...
        for s in message.signatures.iter() {
            ...
            let data = DigestIdentifier::from_serializable_borsh(&event.content)
    ...
    async fn ask_for_signatures(&self, event: &Event) -> Result<(), DistributionManagerError> {
        ...
        targets.remove(&self.signature_manager.get_own_identifier());
        self.send_message(
            event.content.event_proposal,
            DistributionMessages::RequestSignature(RequestSignatureMessage {
                subject_id: event.content.subject_id.clone(),
                namespace: event.content.metadata.namespace.to_owned(),
            ...
            Some(format!(
                "{}/SIGNATURES",
                event.content.subject_id.to_str()
            )),
            Vec::from_iter(targets),
        )
        .await
    ...
    pub async fn set_event(
        ...
        if !self
            .governance
            .check_if_witness(
                message.event.content.metadata.governance_id.clone(),
                message.event.content.metadata.namespace.clone(),
                message.event.content.metadata.schema_id.clone(),
            )
            .await
            .unwrap()
        {
        ...
        match self.db.get_subject(&message.event.content.subject_id) {
            Ok(subject) => {
                ...
                if let Some(signatures) = &message.notaries_signatures {
                    ...
                    if event.content.event_proposal.proposal.sn == subject_data.sn + 1 {
                        ...
                        self.notifier
                            .new_event(&subject_data.subject_id.to_str(), event.content.event_proposal.proposal.sn);
                        return Ok(Ok(()));
                    } else if event.content.event_proposal.proposal.sn > subject_data.sn + 1 {
                        ...
                        let next_sn = if let Some((_, _, next_sn)) = self
                            .synchronization_map
                            .get(&event.content.subject_id)
                        {
                            // Es posible que el sn del evento coincida con next_sn
                            if event.content.event_proposal.proposal.sn == *next_sn {
                                ...
                                if prev_event.signature.content.content_hash
                                    != event.content.previous_hash
                                {
                        ...
                        self.synchronization_map.insert(
                            ...
                            (event.content.event_proposal.proposal.sn, hash, next_sn),
                        );
                    ...
                    if next_sn != event.content.event_proposal.proposal.sn {
                        return Ok(Err(DistributionErrorResponses::EventNotNeeded));
                    }
                    ...
                    let prev_event = match self
                        .db
                        .get_event(&subject_data.subject_id, event.content.event_proposal.proposal.sn - 1)
                    {
                        Ok(prev_event) => prev_event,
                        Err(error) => {
                            return Err(DistributionManagerError::DatabaseError(error.to_string()))
                    ...
                    if sn_to_reach == next_sn {
                        ...
                        if event_hash != event.signature.content.content_hash {
            ...
            Err(DbError::EntryNotFound) => {
                ...
                if event.content.event_proposal.proposal.sn == 0 {
                    ...
                    if let Some(signatures) = &message.notaries_signatures {
                        ...
                        self.notifier
                            .new_subject(&event.content.subject_id.to_str());
                    } else {
                        ...
                        self.send_message(
                            ...
                            Some(format!("{}/EVENT", event.content.subject_id.to_str())),
                            Vec::from_iter(targets),
                        )
                        .await?;
                        return Ok(Ok(()));
                    ...
                    self.synchronization_map.insert(
                        event.content.subject_id.clone(),
                        (event.content.event_proposal.proposal.sn, hash, 0),
                    );
...
//             );
//             prev_hash = event.signature.content.content_hash.clone();
//             subject.apply(&event.content).unwrap();
//             if i == n - 1 {
//                 return event;
//             }
//         }
...
//             return;
//         };
//         assert_eq!(data.event.content.event_proposal.proposal.sn, sn);
//         assert_eq!(data.event.content.subject_id, *subject_id);
//     }
...
//             let prev_hash = genesis_event.signature.content.content_hash.clone();
...
//                 genesis_event.signature.content.content_hash,
//                 3,
//                 &mut subject,
//                 &alt_signature_manager,
//             );
...
//                 .sign(&genesis_event.content)
//                 .unwrap();
//             let third_signature = third_signer.sign(&genesis_event.content).unwrap();
//             let result = manager
...
//                 event_1.signature.content.content_hash.clone(),
//                 1,
//                 &mut subject,
//                 &alt_signature_manager,
//             );
...
//                 event_2.signature.content.content_hash.clone(),
//                 1,
//                 &mut subject,
//                 &alt_signature_manager,
//             );
